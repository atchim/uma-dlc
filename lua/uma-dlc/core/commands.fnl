...
(fn split [s ?sep ?i]
  ...
  `s` must be a string that the iterator will try to split. The splitted string
  will be returned as two values, which are the indexes of the start and the
  end of the split respectively.
...
(fn comp [lead line pos]
  "Complete function for the `UmaDlc` command."
  ...
  (local (subcmd broken?)
    (do
      ...
      (each [_ arg* (ipairs args) &until broken?]
        ...
        (if (= :table (type item)) (set subcmd item) (set broken? true)))
...
(fn setup []
  (vim.api.nvim_create_user_command
    ...
    {:complete comp :count 0 :desc "UmaDlc commands." :nargs :+})

  (let [map vim.keymap.set]
    (map :n "[b" "<Cmd>UmaDlc buf prev<CR>" {:desc "Buffer previous"})
    (map :n "]b" "<Cmd>UmaDlc buf next<CR>" {:desc "Buffer next"})
