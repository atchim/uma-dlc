local function config()
  ...
  return cmp.setup({formatting = {fields = {"kind", "abbr"}, format = _1_}, mapping = {["<C-C>"] = cmp.mapping.abort(), ["<C-E>"] = cmp.mapping(cmp.mapping.scroll_docs(1), {"i", "c"}), ["<C-N>"] = cmp.mapping.select_next_item(), ["<C-P>"] = cmp.mapping.select_prev_item(), ["<C-Y>"] = cmp.mapping(cmp.mapping.scroll_docs(-1), {"i", "c"}), ["<CR>"] = cmp.mapping.confirm({select = true}), ["<Down>"] = cmp.mapping.select_next_item(), ["<S-Tab>"] = cmp.mapping(_2_, {"i", "s"}), ["<Tab>"] = cmp.mapping(_4_, {"i", "s"}), ["<Up>"] = cmp.mapping.select_prev_item()}, snippet = {expand = _6_}, sources = cmp.config.sources({{name = "nvim_lsp"}, {name = "luasnip"}, {name = "buffer"}, {name = "path"}}), view = {entries = {name = "custom", selection_order = "near_cursor"}}})
end
local function _7_()
  return (require("luasnip.loaders.from_vscode")).lazy_load()
end
return {"hrsh7th/nvim-cmp", event = "InsertEnter", config = config, dependencies = {"hrsh7th/cmp-buffer", "hrsh7th/cmp-nvim-lsp", "hrsh7th/cmp-path", {"saadparwaiz1/cmp_luasnip", dependencies = {"L3MON4D3/LuaSnip", config = _7_, dependencies = "rafamadriz/friendly-snippets"}}}}
